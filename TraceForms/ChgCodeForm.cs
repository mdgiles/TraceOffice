...
namespace TraceForms
{
    ...
    public partial class ChgCodeForm : DevExpress.XtraEditors.XtraForm
    {
			...
			try {
				...
				if (modified) {
					...
					if (_selectedRecord.EntityState == EntityState.Detached) {
						_context.CHGCODE.AddObject(_selectedRecord);
					}
		...
		private bool IsModified(CHGCODE record)
		{
		...
		void SetBindings()
		{
			...
			if (BindingSource.Current == null) {
				...
				_selectedRecord = ((CHGCODE)BindingSource.Current);
				SetReadOnly(false);
				SetReadOnlyKeyFields(true);
			}
			ErrorProvider.Clear();
				...
				if (DisplayHelper.QuestionYesNo(this, "Are you sure you want to delete this record?") == DialogResult.Yes) {
					...
					if (!_selectedRecord.IsNew()) {
						...
						if (_selectedRecord != null && (_selectedRecord.EntityState & EntityState.Deleted) != EntityState.Deleted)
							_context.CHGCODE.DeleteObject(_selectedRecord);
						_context.SaveChanges();
					}
		...
		void ClearBindings()
		{
			BindingSource.DataSource = typeof(CHGCODE);
		}
		...
		private void ExecuteQuery()
		{
			...
			var records = _context.CHGCODE.Where(query);
			if (records.Count() > 0) {
		...
		private void CheckEditConfirm_Click(object sender, System.EventArgs e)
        {
