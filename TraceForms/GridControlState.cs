...
namespace FlexForms
{
    public class GridControlState {
        ...
        public class ViewState {
            ...
            private static ViewState Create(ViewState parent, ColumnView view) {
            ...
            protected int GetRowHandleToSelect(ColumnView view, RowInfo rowInfo) {
                int dataRowHandle = LocateRowByKeyValue(view, rowInfo.Id);
                if (dataRowHandle != GridControl.InvalidRowHandle)
                    if (view is GridView && ((GridView)view).GetRowLevel(dataRowHandle) != rowInfo.level)
                        return FindParentRowHandle((GridView)view, rowInfo, dataRowHandle);
                return dataRowHandle;
            }

            protected void SelectRowByRowInfo(ColumnView view, RowInfo rowInfo, bool isFocused) {
                int rowHandle = GetRowHandleToSelect(view, rowInfo);
                if (isFocused)
                    view.FocusedRowHandle = rowHandle;
                else {
                    if (view is GridView && ((GridView)view).OptionsSelection.MultiSelectMode == GridMultiSelectMode.CellSelect) {
                        string[] names = CellSelection[rowInfo.Id] as string[];
                        if (names != null) {
                            for (int j = 0; j < names.Length; j++) {
                                ((GridView)view).SelectCell(rowHandle, view.Columns[names[j]]);
                            }
                        }
                    } else {
            ...
            public void SaveSelectionViewInfo(ColumnView view) {
                SaveSelList.Clear();
                CellSelection.Clear();
                ...
                if (selectionArray != null)  // otherwise we have a single focused but not selected row
                    for (int i = 0; i < selectionArray.Length; i++) {
                        ...
                        if (view is GridView && ((GridView)view).OptionsSelection.MultiSelectMode == GridMultiSelectMode.CellSelect) {
                            GridColumn[] columns = ((GridView)view).GetSelectedCells(dataRowHandle);
            ...
            private int AddRowToSelection(ColumnView view, int handle) {
                RowInfo rowInfo = new RowInfo();
                if (view is GridView) {
                    rowInfo.level = ((GridView)view).GetRowLevel(handle);
                    if (handle < 0) // group row
                        handle = ((GridView)view).GetDataRowHandleByGroupRowHandle(handle);
                }
            ...
            public void SaveExpandedMasterRows(GridView view) {
                ...
                for (int i = 0; i < view.DataRowCount; i++)
                    if (view.GetMasterRowExpanded(i)) {
                        ...
                        ColumnView detail = view.GetVisibleDetailView(i) as ColumnView;
            ...
            public void LoadSelectionViewInfo(ColumnView view) {
                view.BeginSelection();
                try {
                    view.ClearSelection();
                ...
                try {
                    ...
                    for (int i = 0; i < SaveMasterRowsList.Count; i++) {
                        ...
                        if (state == null) {
                            ...
                            ColumnView detail = view.GetVisibleDetailView(rowHandle) as ColumnView;
                            if (detail != null) {
            ...
            private int LocateRowByKeyValue(ColumnView view, object value) {
            ...
            public void SaveState(ColumnView view) {
                if (ReferenceEquals(view.GridControl.FocusedView, view)) gridState.focused = this;
                if (view is GridView) {
                    GridView gridView = (GridView)view;
                    SaveExpandedMasterRows(gridView);
                    SaveExpansionViewInfo(gridView);
                    SaveVisibleIndex(gridView);
                    horzScrollPos = (gridView).LeftCoord;
                }
                SaveSelectionViewInfo(view);
            }

            public void LoadState(ColumnView view) {
                if (view is GridView) {
                    GridView gridView = (GridView)view;
                    LoadExpandedMasterRows(gridView);
                    LoadExpansionViewInfo(gridView);
                    LoadVisibleIndex(gridView);
                    gridView.LeftCoord = horzScrollPos;
                }
